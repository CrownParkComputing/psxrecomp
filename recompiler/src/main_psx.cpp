...
std::filesystem::path out_dir = "generated";
    ...
    // If --splitfunc is specified, write one C file per function
    if (split_per_function) {
        std::filesystem::create_directories(out_dir);

        // First, write the shared declarations header
        ...
        // Remove stale per-function outputs
        std::error_code rm_ec;
        for (int stale = 0; stale < 256; stale++) {
            std::filesystem::path stale_shard =
                out_dir / fmt::format("{}_func_{:02d}.c", exe_stem, stale);
        ...
        for (const auto& gf : gen_funcs) {
            ...
            if (func_file.is_open()) {
                func_file << fmt::format("/* Generated by PSXRecomp - {}. DO NOT EDIT. */\n", func_filename.filename().string());
                func_file << fmt::format("#include \"{}_decls.h\"\n\n", exe_stem);
                func_file << gf.full_code << "\n";
                func_file.close();
                fmt::print("✓ Saved function {} to {}\n", gf.function_name, func_filename.string());
            } else {
                fmt::print(stderr, "⚠ Failed to write function file for {}\n", gf.function_name);
            }
            func_index++;
        }

        fmt::print("✓ Wrote {} function files for {}\n\n", func_index, exe_stem);
    } else {
