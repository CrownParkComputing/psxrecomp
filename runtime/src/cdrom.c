...
static int cd_audio_sector_size;  /* 2352 for raw audio sectors */

/* CD-DA (Red Book) audio playback state */
static int      cd_audio_playing;
static int      cd_audio_track;
static int      cd_audio_lba;
static int      cd_audio_end_lba;
...
/* Convert LBA to track number (1-based, 0 if not found) */
static int lba_to_track(int lba) {
    if (!iso_handle) return 0;
    int tracks = iso_track_count(iso_handle);
    for (int t = 1; t <= tracks; t++) {
        uint32_t start = iso_track_start_lba(iso_handle, t);
        uint32_t next_start = iso_track_start_lba(iso_handle, t + 1);
        if (next_start == 0) next_start = iso_sector_count(iso_handle);
        if (lba >= (int)start && lba < (int)next_start) return t;
    }
    return 0;
}
...
case 0x03: /* Play — start CD-DA audio playback (from SetLoc, or optional
        ...
        cd_audio_lba = msf_to_lba(seek_min, seek_sec, seek_sect);
        cd_audio_track = lba_to_track(cd_audio_lba);
        if (cd_audio_track > 0 && iso_track_is_audio(iso_handle, cd_audio_track)) {
            cd_audio_end_lba = iso_track_start_lba(iso_handle, cd_audio_track + 1);
            if (cd_audio_end_lba == 0) cd_audio_end_lba = iso_sector_count(iso_handle);
            cd_audio_playing = 1;
            xa_reset_decode();
            spu_cd_audio_reset();
            cd_audio_sector_size = RAW_SECTOR_SIZE;
        }
        break;
...
static void process_read_stream(uint32_t cycles) {
     ...
     * While Play (0x03) is active and we're on an audio track, continuously
     * feed raw 2352-byte sectors to the SPU CD input ring at 75 sectors/sec. */
    if (cd_audio_playing && cd_audio_lba < cd_audio_end_lba && iso_handle) {
        uint8_t raw_data[RAW_SECTOR_SIZE];
        if (iso_read_raw_sector(iso_handle, cd_audio_lba, raw_data, RAW_SECTOR_SIZE)) {
            /* Red Book CD-DA: 2352 bytes of raw interleaved stereo PCM,
             * no sync/header/subheader to skip (unlike data sectors). */
            int16_t stereo[1176]; /* 588 stereo samples = 2352 bytes */
            int frames = 0;
            for (int i = 0; i < 2352; i += 4) {
                int16_t l = (int16_t)(raw_data[i] | (raw_data[i + 1] << 8));
                int16_t r = (int16_t)(raw_data[i + 2] | (raw_data[i + 3] << 8));
                stereo[frames * 2] = l;
                stereo[frames * 2 + 1] = r;
                frames++;
            }
            spu_cd_audio_push(stereo, frames);
        }
        cd_audio_lba++;
        /* Auto-stop at end of track or end of disc */
        if (cd_audio_lba >= cd_audio_end_lba) {
            cd_audio_playing = 0;
            cd_audio_lba = cd_audio_end_lba = 0;
            cd_audio_track = 0;
            spu_cd_audio_reset();
            stat_reg = (stat_reg & ~CDSTAT_PLAY) | CDSTAT_MOTOR;
        }
    }
}
